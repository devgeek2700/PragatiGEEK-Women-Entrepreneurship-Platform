        ...
        <HomeLayout>
            <div className="flex flex-col lg:flex-row lg:px-20 py-12 bg-gray-100 rounded-lg shadow-lg">
                <div className="lg:w-1/2 w-full px-12 flex flex-col gap-7">
                    <img src={state.thumbnail?.secure_url} alt="thumbnail" className="rounded-xl w-full h-96 object-cover" />
                    <div className="bg-white p-4 rounded-lg shadow-md">
                        ...
                        <p className="font-semibold lg:text-2xl text-xl text-yellow-400 capitalize">Price: <span className="text-xl text-blue-500">₹{state.price}</span></p>
                        {
                            state.isEnrolled ? (
                                <button className="btn btn-primary capitalize mt-4" onClick={() => navigate(`/course/${state.title}/${state._id}/lectures`, { state: state })}>Go to Lectures</button>
                            ) : data?.subscription?.status === 'active' ? (
                                <button className="btn btn-primary capitalize mt-4" onClick={() => navigate(`/course/${state.title}/${state._id}/lectures`, { state: state })}>Go to Lectures</button>
                            ) : (
                                <button className="btn btn-primary capitalize mt-4" onClick={() => navigate(`/course/${state.title}/checkout`, { state: state })}>Subscribe</button>
                            )
                        }
                    </div>
                </div>
                <div className="lg:w-1/2 w-full px-12 py-12 flex flex-col gap-4 ">
                    <h1 className="font-bold text-yellow-500 lg:text-4xl text-2xl capitalize">{state.title}</h1>
                    <p className="font-semibold lg:text-2xl text-xl text-amber-500 capitalize">Course Description:</p>
                    <p className="font-semibold lg:text-xl text-base text-gray-700 normal-case tracking-wider">{state.description}</p>
                </div>
            </div>
        </HomeLayout>
    )
