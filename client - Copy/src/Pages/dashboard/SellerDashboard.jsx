...
import { BsBox, BsGraphUp, BsShop, BsWallet2, BsCashStack } from "react-icons/bs";
...
const SellerDashboard = () => {
    ...
    const chartData = {
        labels: stats.monthlyRevenue?.length > 0 
            ? stats.monthlyRevenue.map(item => `${item._id?.month || 'N/A'}/${item._id?.year || 'N/A'}`) 
            : ['No Data'],
        datasets: [
            {
                label: 'Revenue (₹)',
    ...
    const calculateMonthlyGrowth = () => {
        ...
        const currentMonth = stats.monthlyRevenue[stats.monthlyRevenue.length - 1].total || 0;
        const lastMonth = stats.monthlyRevenue[stats.monthlyRevenue.length - 2].total || 1;
                                    ...
                                    {stats.recentOrders.map((order) => (
                                        <tr key={order._id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm font-medium text-gray-900">{order.buyer?.name || 'N/A'}</div>
